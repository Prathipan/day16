...
request.onload = function(){
    ...
    function pageButtons(pages){
        ...
        for(let page = maxLeft; page <= maxRight ; page ++){
            wrapper.innerHTML = wrapper.innerHTML + `<button value="${page}" class="page btn btn-primary">${page}</button>`
            
        }
        if(state.page!==1){
            wrapper.innerHTML = `<button value=${1} class="page btn btn-primary">&#171; First</button>` + wrapper.innerHTML
        }
        if (state.page != pages) {
